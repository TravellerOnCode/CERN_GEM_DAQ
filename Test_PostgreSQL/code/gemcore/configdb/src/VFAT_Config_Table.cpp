...
void VFAT_Config_Table::create_table(connection *dbClient) {
  ...
  string query = "CREATE TABLE IF NOT EXISTS " + to_string(VFAT_CONFIG_TABLE) +
...
void VFAT_Config_Table::insert_row(connection *dbClient) {
  ...
  string sql = "INSERT INTO " + to_string(VFAT_CONFIG_TABLE) + " VALUES(" +
               to_string(this->config_id) + ");";
...
void VFAT_Config_Table::insert_data(connection *dbClient,
                                    vector<VFAT_Config_Table> data) {
  int i;
