...
vector<VFAT_Data_Table> Postgres_Database::getVFATSettings(string filename) {
  ...
  for (auto vfat_ele = j.begin(); vfat_ele != j.end(); vfat_ele++) {
    ...
    // cout << "VFAT_ID -> " << vfat_id << "CFG_IREF -> " <<
    // field_data["CFG_IREF"] << endl;
...
vector<VFAT_Data_Table>
Postgres_Database::getReferenceVFATSettings(connection *dbClient,
  ...
  // vector<long> id_list;
  work WW(*dbClient);
  string query = "SELECT * FROM " + to_string(VFAT_INDEX_TABLE) +
                 " WHERE CONFIG_ID = " + to_string(reference_config_id) + ";";
  string id_list = "";
  result r = WW.exec(query);
  ...
  query = "SELECT * FROM " + to_string(VFAT_DATA_TABLE) + " WHERE ID IN (" +
          id_list + ");";
  r = WW.exec(query);
