...
void VFAT_Index_Table::create_table(connection *dbClient) {
  ...
  string query = "CREATE TABLE IF NOT EXISTS " + to_string(VFAT_INDEX_TABLE) +
                 "( CONFIG_ID bigint, ID bigint,";
  string s1 = "PRIMARY KEY(CONFIG_ID,ID),";
  string s2 = "FOREIGN KEY (CONFIG_ID) REFERENCES " +
              to_string(VFAT_CONFIG_TABLE) + "(CONFIG_ID),";
  string s3 =
      "FOREIGN KEY (ID) REFERENCES " + to_string(VFAT_DATA_TABLE) + "(ID));";
...
void VFAT_Index_Table::insert_row(connection *dbClient) {
  ...
  string sql = "INSERT INTO " + to_string(VFAT_INDEX_TABLE) + " VALUES(" +
               to_string(this->config_id) + ", " + to_string(this->id) + ");";
...
void VFAT_Index_Table::insert_data(connection *dbClient,
                                   vector<VFAT_Index_Table> data) {
  int i;
